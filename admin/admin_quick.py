...
admin_quick = Blueprint('admin_quick', __name__, url_prefix='/admin/quick_shop' ,template_folder='templates',static_folder='static')

@admin_quick.route('/', methods=['GET', 'POST'])
def credit_goods():
...
#添加積分商品
@admin_quick.route('/add_credit/<string:file>', methods=['GET','POST'])
def add_credit(file):
    if not session.get('admin'):
        abort(400)
    form = Add_photo()
...
@admin_quick.route('/del_credit/<string:file>', methods=['GET', 'POST'])
def del_credit(file):
    if not session.get('admin'):
        abort(400)
...
#admin查看用戶兌換情況
@admin_quick.route('/goods', methods=['GET','POST'])
def admin_goods():
    if not session.get('admin'):
        abort(400)
