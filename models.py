...
class Coupon(db.Model):
    ...
    id = db.Column(db.String(64), unique=True)
    active_state = db.Column(db.Boolean, default=True)
    create_date = db.Column(db.DateTime, default=(datetime.now + timedelta(hours=8)))
    use_date = db.Column(db.DateTime, default=(datetime.now + timedelta(hours=8)), onupdate=(datetime.now + timedelta(hours=8)))
